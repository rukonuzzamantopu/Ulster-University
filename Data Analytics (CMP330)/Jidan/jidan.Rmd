...
```{r}
table(car_data$acceptability)["acc"]
```



```{r}
table(Buying = car_data$buying, Acceptability = car_data$acceptability)
```

```{r}
# Cross-tabulation: buying price vs acceptability
buy_table <- table(car_data$buying, car_data$acceptability)
buy_table

# Extract the specific cell: high buying price, "good" acceptability
buy_table["high", "good"]
```


```{r}
mutinformation(car_data$persons, car_data$acceptability)
```


```{r}
mi_matrix <- mutinformation(car_data[, 1:6])  # exclude acceptability
mi_matrix

# Find the max off-diagonal value
diag(mi_matrix) <- 0
which(mi_matrix == max(mi_matrix), arr.ind = TRUE)
```
```{r}
chisq.test(car_data$persons, car_data$doors)
```
```{r}
prop.table(table(car_test$acceptability))["vgood"]
```
...
nn_model <- nnet(acceptability ~ ., data = car_train, size = 3)
nn_pred  <- predict(nn_model, car_test, type = "class")
nn_pred  <- factor(nn_pred, levels = levels(car_test$acceptability))
