    ...
    public void AdjustmentStage(GameState state) {
        ...
        for (Player P : state.Players) {
            ...
            if (P.IsComputer()) {
                ...
                System.out.println(" Computer will discard " + ToDiscard + " card(s).");

                // Human decision: prompt whether to replace and how many
            } else {
                System.out.print("Would you like to replace cards? (y/n): ");
                String Ans = state.Scanner.nextLine().trim().toLowerCase();
                if (!Ans.isEmpty() && Ans.charAt(0) == 'y') {
                    int Max = Math.min(2, P.GetCollected().size());
                    if (Max == 0) {
                        System.out.println("You have no collected cards available for replacement.");
                        ToDiscard = 0;
                    } else {
                        int Temp = -1;
